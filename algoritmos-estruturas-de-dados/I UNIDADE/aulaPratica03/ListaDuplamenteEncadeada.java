public class ListaDuplamenteEncadeada<T>{
    ...
    public ListaDuplamenteEncadeada(){}; //Por definicao, head e tail sao inicializadas nulas, desse modo, a lista tambem comeca nula, pois nao ha nada para os ponteiros apontarem

    /*METODOS OBRIGATORIOS**************************************************************************************************************************************************************/

    public void inserirInicio(T item){
        Elemento<T> temp = new Elemento<T>(item, null, head); //Instanciacao de um elemento temp com dado = item, com antecessor null e apontando para head para adquirir primeira posicao na lista 
        if(head == null){ //Se a variavel head esta vazia, entao temp aponta para null e acaba sendo tambem o ultimo da lista
            ...
            head.setAnterior(temp); //Se a lista nao estiver vazia, entao eh modificado o elemento anterior do antigo primeiro elemento, para onde head aponta
        }
    ...
    public void inserirFim(T item){
        Elemento<T> temp = new Elemento<T>(item, tail, null); //Instanciacao de um elemento temp com o dado = item, com antecessor tail e apontando para null, pois agora eh o ultimo elemento da lista e nao tem sucessor
    ...
    public void remover(T item){
        ...
        while(aux != null && !(aux.getDado().equals(item))){ //Estrutura condicional de verificacao para: enquanto o elemento auxiliar nao apontar para nulo, ou seja, enquanto nao for o ultimo da lista; e enquanto o dado de aux nao for igual ao item passado como parametro no metodo:
            aux = aux.getProximo(); //Progressao do elemento auxiliar na lista com aux apontando para o sucessor do elemento que estava apontando anteriormente
        }
        ...
        if(aux == head){ //Se o elemento auxiliar apontar para head, apos o while, significa que o elemento com dado = item esta na primeira posicao
            ...
            aux.setAnterior(null); //Agora o antecessor de aux, e consequentemente head tambem, eh null
            ...
            tail = aux.getAnterior(); //Agora tail vai apontar para o elemento antecessor ao elemento auxiliar, que agora nao sera mais o penultimo, mas o ultimo, excluindo aux da lista encadeada
            tail.setProximo(null); //O anterior a aux, que agora eh o ultimo da lista, aponta para null, excluindo aux da lista
            tamanho--; //Atualiza o valor do tamanho da lista para tamanho-1, ao excluir um elemento
        }
        else{ //Caso nao seja o primeiro, nem o ultimo elemento, apos o while, entao:
            aux.getAnterior().setProximo(aux.getProximo()); //O elemento anterior ao auxiliar agora vai apontar para o sucessor de aux, tornando aux.proximo o sucessor direto de ant.amterior e, dessa forma, removendo aux da lista encadeada
            tamanho--; //Atualiza o valor do tamanho da lista para tamanho-1, ao excluir um elemento
        }
    }
    ...
    public void exibirLista(){
        ...
        while(aux != null){ //Estrutura de condicao para percorrer completamente a lista encadeada, com verificacao se o elemento auxiliar nao aponta para null, ou seja, a lista foi percorrida
            System.out.print("[" + aux.getDado() + "] ⇆"); //Imprime o dado de aux a cada repeticao do laco entre colchetes com um encadeamento de duas setas paralelas e divergentes em sentido pra representar o duplo encadeamento
